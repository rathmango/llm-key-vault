...
여러 LLM 서비스(OpenAI/Anthropic/Gemini/OpenRouter/Ollama 등)를 사용할 때, API Key는 각 앱/클라이언트에 흩어져 저장되고 관리가 어렵습니다. 이 프로젝트는 **macOS + iPadOS(Apple Silicon/M1+)**에서 동작하는 앱으로, **여러 LLM Provider의 API Key를 안전하게 저장**하고 **한 앱에서 즉시 전환/동시 실행(비교 실행)**할 수 있게 합니다.

추가로(선택 기능), macOS에서는 로컬에서 **OpenAI 호환 프록시(로컬 엔드포인트)**를 제공해 다른 도구들이 API Key를 직접 저장하지 않고도 사용할 수 있게 합니다.
...
- **G1. 안전한 Key 관리**: Keychain 기반 저장(기본) + 선택적으로 iCloud 동기화.
- **G2. 단일 앱에서 Provider 전환**: Provider/모델을 빠르게 전환해 사용.
- **G3. 동시 실행/비교**: 한 프롬프트를 여러 Provider/모델에 동시에 보내 결과를 나란히 비교.
- **G4. (옵션) macOS 로컬 프록시**: 다른 클라이언트가 로컬 엔드포인트를 통해 Key를 간접 사용.
