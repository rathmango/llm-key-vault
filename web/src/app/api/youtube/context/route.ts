...
import { geminiTranscribeYouTubeUrl } from "@/lib/gemini";
...
export async function GET(request: Request) {
  try {
    ...
    const [video, transcriptPrimary] = await Promise.all([
      fetchVideoMeta(apiKey, videoId),
      fetchTranscript(videoId, parsed.lang),
    ]);

    let transcript: Transcript | null = transcriptPrimary;
    let transcriptSource: "timedtext" | "gemini" | "none" = transcriptPrimary ? "timedtext" : "none";

    // If captions are not available via timedtext, fallback to Gemini Video Understanding
    // to generate a timestamped transcript (Lilys-style “always transcript” behavior).
    if (!transcript) {
      const geminiKey = await loadGeminiKey(user.id);
      if (geminiKey) {
        const t = await geminiTranscribeYouTubeUrl({
          apiKey: geminiKey,
          youtubeUrl: `https://www.youtube.com/watch?v=${encodeURIComponent(videoId)}`,
          languageHint: parsed.lang ?? "ko",
          model: "gemini-2.5-flash",
        });

        const MAX_CHARS = 120_000;
        let textWithTimestamps = t.textWithTimestamps;
        let isTruncated = false;
        if (textWithTimestamps.length > MAX_CHARS) {
          textWithTimestamps = textWithTimestamps.slice(0, MAX_CHARS);
          isTruncated = true;
        ...
        const segmentsCount = textWithTimestamps
          .split("\n")
          .map((x) => x.trim())
        ...
        transcript = {
          language: parsed.lang ?? t.language,
    ...
    return Response.json({
      ...
      meta: {
        ...
        transcriptSource,
        geminiFallbackEnabled: transcriptSource === "gemini",
      },
    });
  } catch (e) {
